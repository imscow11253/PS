...
### 출력 
 ...
 <p>어려웠다. cpp의 STL의 내부 구현을 무지성으로 사용만 하던 것이 독이된 문제의 대표다. 처음엔 set이 내부에 red-black tree로 구현되어 있고 이진탐색인 upper_bound가 제공되니까 set에 넣고, upper_bound로 찾고 erase 연산도 Logn 이니까 괜찮을 줄 알았는데 시간초과가 났다. 왜? 시간복잡도는 문제가 없는데? set에 들어가는 요소의 개수가 커질수록 red-black tree 구조의 balance를 유지하는 비용이 많이 들어서 erase, insert가 logn이 아닐 수 있다는 것이다. 이럴 땐 어떻게 해야할지 모르겠어서 결국 다른 사람 풀이를 보았다. union-find를 정렬되어 있는 vector에 써서 vector의 요소를 그룹 짓는 것이다. 신기했다. 앞으로 이런 개념을 잘 활용해보도록 하자. </p>
