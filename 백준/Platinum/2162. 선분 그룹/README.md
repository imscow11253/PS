...
### 출력 
 ...
 <p>두 선분 교차 로직을 저번에 풀어보았어서 그걸 사용했고, 같은 그룹인지 판단하는건 union-find로 풀었다. 마지막에 각 node 별로 find를 한 번 씩 돌려서 parent를 update해줘야 한다. 그리고 union-find 구현을 대충 익히고 있었는지 구현에서 살짝 미스가 있었다. </p>
