...
### 풀이 
 ...
 <p>전형적인 백트래킹 문제인데 대각선으로 이동하면서 visit 체크를 해야한다. 나는 풀다가 모르겠어서 그냥 제미나이한테 물어보았다. 우선 비숍은 체스판에서 하얀색에 있으면 하얀색만 이동가능하고, 검은색에 있으면 검은색만 이동가능하다. 그래서 2개를 따로따로 생각하고 각각의 최대를 더하면 된다. 그리고 백트래킹을 내가 잘못 이해했는지 변질되었는지 이상하게 구현하고 있었다. 시작점 하나만 함수 호출하면 된다. 모든 시작점을 기준으로 안해도 된다. 놓을 건지 안놓을건지 내부적으로 구현하면 된다. 그리고 대각선 판단을 신기하게 했다. 같은 대각선에 있으면 row+col 값이 같다. 그리고 반대 대각선은 row-col 이 같다. 그래서 대각선 마다 고유 인덱스를 나타내는 visit 함수를 두어서 해당 대각선에 위치하는 기물을 놓을 수 있는지 없는지를 판단하는 방식을 사용했다. 2차원 배열에서 대각선 놓는 것 좀 알아봐야겠다. </p>
