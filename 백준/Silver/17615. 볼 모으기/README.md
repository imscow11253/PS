...
### 출력 
 ...
 <p>규칙만 알면 O(N)에 풀 수 있다. 최종 case가 어떻게 되는지 살펴보면 좋은데 빨강은 전부 왼쪽, 파랑은 전부 오른쪽 이거나 반대의 경우일 수 밖에 없다. 각각을 만들기 위해 빨간색을 옮기는 경우, 파란색을 옮기는 경우 2가지가 있다. 그러면 최종 경우의 수는 4개이고 각각 O(N)으로 탐색이 가능하다. 끝의 공이 목표 하는 색깔이면 연속된 공은 세지 않아도 된다. </p>
