...
### 풀이
 ...
 <p>조금 헷갈렸다.. n 제한이 10,000이라서 O(n^2) 에 풀릴 줄 알고 그냥 브루트포스 방식으로 도전했다가 시간초과가 났다. m 을 고려하지 않아서 생긴 문제였고 완전탐색을 하게 되면 O(n^2 * m)이 되어서 시간초과가 난다. 

뭔가 알고리즘적으로 시간을 줄이는 것보단 문제에서 특징을 파악해서 탐색의 범위를 줄이는 방식으로 풀어야 할 것 같았고 두 번째로 시도해본건 각 점의 평균 값이다. 근데 틀렸다고 한다. 

결국 원인을 찾지 못해서 구글에 검색해보니 각 점의 중간값이라고 한다. 수학 식을 세우고 그래프로 증명하는데 아직도 잘 와닿지는 않는다. 그냥 문제 유형 외우는게 나을 듯. **점 모으기 방식의 문제는 중앙값을 써야 한다.**</p>
