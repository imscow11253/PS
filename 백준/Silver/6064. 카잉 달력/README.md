...
### 출력 
 ...
 <p>체감상 진짜 어려웠다. 아니 돌아보면 그렇게 어렵진 않았는데 내가 뻘짓 좀 한 것 같다. 그냥 1 ~ 최소공배수까지 투 포인터로 반복하면서 나머지가 x:y와 일치하는 걸 찾으면 시간초과 난다는 것은 바로 알았다. 뭔가 규칙이 보이는 것 같아서 노가다 하면서 규칙찾는데 삽질 좀 했다. 근데 결국 규칙 못찾고 규칙 찾으면서 직접 경우의 수를 줄여가면서 노가다 했던 방식을 그대로 사용했더니 됐다. 처음에 edge case 생각 못해서 한 번 틀렸었었다. </p>
