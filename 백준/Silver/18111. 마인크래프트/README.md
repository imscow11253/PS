...
### 출력 
 ...
 <p>은근 생각을 좀 해야하는 문제다. 간단하게 브루트포스로 풀었다. 최종 정리된 땅의 높이는 입력으로 들어온 값의 최소 ~ 최대까지 중에 하나로 결정된다. 근데 해당 높이로 맞추기 위한 블록의 개수가 부족할 수 있다. 전체적으로 보면 입력 값들의 합 + b의 값보다 최종 높이의 개수가 크면 가능한 조합이다. 가능한지 안되는지 판단하고 time을 브루트포스로 계산하면 된다. </p>
