...
### 풀이 
 ...
 <p>어렵지 않았지만 문제를 단순화해서 풀어야 한다. 출발점과 도착점이 특정 원안에 있으면 경계를 무조건 지나야 한다. 근데 두 점이 동시에 같은 원 안에 있으면 경계를 지날 필요가 없다. xor 연산을 생각해서 풀었다. 비트 마스킹을 GPT 도움 없이 스스로 생각해서 풀었다는 점이 대견하다. </p>
