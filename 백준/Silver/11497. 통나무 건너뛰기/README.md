...
### 풀이 
 ...
 <p>처음 봤을 땐 어려워 보였다. 배열의 순서를 전부 다 체크 해봐야 하나 싶어서 순열 문제인줄 알았다. 순열을 어떻게 구현해야 하나 했는데 시간 내에 풀릴 것 같지 않았다. 질문게시판을 보니 누가 힌트를 주었다. 정규분포표를 힌트라고 주었다. 조금 생각해보니 각 요소간의 차이를 최소로 하려면 정규분포와 같은 배열이 되어야 된다는 것을 깨달았다. </p>
