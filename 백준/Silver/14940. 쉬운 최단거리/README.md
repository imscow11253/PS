...
### 출력 
 ...
 <p>문제를 그대로 읽으면 각 지점에서 dfs나 bfs를 돌려서 도착 지점까지 최소 거리를 계산해야 할 것처럼 느껴진다. 하지만 그러면 시간초과가 날 것이라는 것을 알 수 있다. 1000*1000*1000*1000 이 되기 때문이다. 조금만 생각해보면 간단하다. 도착지점에서 bfs를 돌려서 거리를 같이 queue에 넣어두었다가 결과 배열에 쓰면 된다. </p>
