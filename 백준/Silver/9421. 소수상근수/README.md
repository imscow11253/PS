...
### 출력 
 ...
 <p>소수 판정은 에라토스테네스의 체를 써야 한다. (자꾸 최대공약수를 구하는 유클리드 호제법과 헷갈린다.) 소수를 미리 다 구해두고 dp 식으로 구했다. 어떤 수에 대해서 상근수인지 판별하고 상근수가 아니면 그 수를 구하는 과정에서 나온 수들을 집합에 넣어둔다. 나중에 다른 수를 판별할 때 그 집합에 있는 수가 나오면 상근수가 아님을 한 번에 증명한다. </p>
