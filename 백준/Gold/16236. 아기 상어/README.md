...
### 풀이 
 ...
 <p>처음엔 그냥 BFS만으로 풀 수 있을 것 같았다. 그런데 구현하다보니 우선순위를 계산할 수 없을 것이라 판단하고 우선순위 큐를 두었다. bfs로 먹을 수 있는 모든 물고기를 탐색하고 물고기를 우선순위 큐에 넣어서 가장 우선순위가 높은 물고기를 먹어나가는 과정으로 진행했다. n이 최대 20으로, naive하게 진행해도 시간초과가 나지 않는다. </p>
