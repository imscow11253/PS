...
### 출력 
 ...
 <p>dp 문제다. 처음 브루트 포스 (BFS) 로 생각 해보면 2^4000이라서 시간초과가 날 것 같았다. 그래서 DP를 생각했다. 근데 DP 를 적용하는게 맞는가 애매하기도 했고, 어떻게 dp를 저장해나갈지 몰라서 일단 규칙을 파악해보려 했다. 

근데 그러면 그냥 2^d가 답 아닌가? 했는데 중간에 땅에 닿으면 안된다. 그래서 각각의 경우의 수를 다 봐야했다. 그래서 모든 경우를 다 봐야하는거다. BFS는 시간초과 나니까 DP를 떠올리면 된다. 

거리와 고도를 2차원 배열로 만들고 dp[i][j] = dp[i-1][j-1] + dp[i+1][j-1] 점화식으로 풀면 된다.</p>
