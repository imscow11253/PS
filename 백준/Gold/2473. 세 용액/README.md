...
### 출력 
 ...
 <p>못 풀었다. '두 용액'이라고 골드에서 투포인터를 써서 푸는 문제와 비슷한데 여기선 세용액이다. 그래서 투포인터가 아니라 쓰리 포인터를 써야 하나하고 각 포인터가 이동하는 조건을 계산하려 노력했는데 결국 못찾았다. gpt한테 물어보니, 값 하나를 고정하고 나머지에 대해서 투포인터를 하라고 했다... 똑똑하다. 그리고 투포인터 시작 시점과 이동하는 조건을 잘 계산해야 한다. 투포인터 더 공부해야 할 것 같다. 네이버 코테나 신한은행 코테에서도 나온 단골 문제이다. </p>
