...
### 출력 
 ...
 <p>예전에 블로그에서 BFS와 DP는 서로 호환가능하다는 글을 읽은 적이 있다. 이 점이 항상 알고리즘을 떠올리게 하는데 큰 도움을 준다. 이번 문제도 naive하게 생각하면 BFS이다. 근데 골드 4에 쫄아서 "BFS일리 없어!" 하면서 DP로 시도했는데 딱봐도 넘 어려워보였다. 그리고 path를 알아야해서 dp는 쉽지 않다. 그래서 단순하게 보니 BFS로 해도 시간초과 안 날 것 같아서 BFS로 풀었다. n 이 0일 수 있다는 점을 고려하지 않아서 초반에 몇 번 틀렸다. </p>
