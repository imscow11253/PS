...
### 출력 
 ...
 <p>단순 bfs 탐색 문제여서 구현하다가 모든 bfs 큐의 노드마다 visit 정보를 별도로 가지고 있어야 한다는 점을 깨달았다. 그때마다 항상 딜레마인게, 큐에 visit 정보도 같이 넣으면 무조건 메모리 초과 나서 어떻게 풀어야 하나 고민했었는데 GPT가 visit 배열을 3차원으로 두고 하나의 차원에 대해서 key 비트마스킹을 사용하라고 했다. 그렇게 하니 되었다! key를 얻으면 visit 배열은 차원이 하나 바뀌면서 초기화 된 느낌이기 때문이고, 이전 차원에 변경 영향을 안준다~!~! 와우와우 visit 배열을 3차원으로 할 생각은 전혀 못했다. God GPT</p>
