...
### 풀이 
 ...
 <p>BFS의 변형 문제였다. 추가적으로 자료구조를 vector, pair, set, queue 등 여러가지 많이 썼다. 0인 부분에 대해서 각 블록의 크기를 bfs로 계산해두고, 1인 블록을 순회하면서 인접한 0이 있으면 해당 블록의 개수만큼 더했다. 근데 같은 블록에 해당하는 0인 칸이 동시에 1에 인접할 수도 있으므로, 블록마다 고유의 id 값을 주어서 해결했다. </p>
