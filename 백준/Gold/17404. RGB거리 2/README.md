...
### 풀이 
 ...
 <p></p>일반적인 RGB 문제에서 조건이 추가된 문제다. 처음과 끝의 집도 맞닿아 있다고 생각하고 색이 겹치지 않아야 하는 것이다. 처음에는 어떻게 풀어야 하나 감이 안잡혔다. 1번 집에서 골라진 색깔 정보를 dp에 계속 저장해야 하나, 아니면 맨 끝에 첫번째 집 정보를 포함하는 셀을 하나 추가해야 하나 싶었는데 해답은 단순했다. 

첫 번째 집의 색깔을 정해두고 각각 dp를 돌려서 최솟값을 출력하면 되는 간단한 문제였다.
