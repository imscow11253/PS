...
### 풀이 
 ...
 <p>요즘 맨날 bfs, dfs, 최단거리 문제만 푸니까 사고회로가 그쪽으로만 돌아가는 것 같다. 처음에 bfs로 풀었다가 방문 처리를 잘못하고 있단 걸 깨닫고, 중복 방문이 가능하게 하니 메모리 초과가 떴다. 당연하다. 결국 모르겠어서 태그를 보니 백트래킹이었다! 조금만 더 생각했으면 알았을 것 같은데 백트래킹을 다시 감 잡도록 해야겠다. </p>
