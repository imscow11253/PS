...
### 풀이  
 ...
 <p>딱봐도 전형적인 dp 문제인데 이게 왜 골드문제일까 싶었다. 제한을 보니 메모리가 4MB 밖에 안되는 것이었다. 이건 입력을 int arr[100000][3] 으로만 받아도 제한을 넘어서기 때문에 2차원 배열을 쓸 수가 없었다. 그래서 1,2,3 열 마다 변수를 만들어두고 이전 dp 값만 기억했다가 계산하는 방식으로 했다. </p>
