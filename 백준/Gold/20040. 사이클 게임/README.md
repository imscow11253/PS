...
### 출력 
 ...
 <p>그래프에서 사이클을 판단하는 문제였다. 이런 유형의 문제를 처음 알았고 알고리즘 공부를 했다. union-find를 통해 풀어도 되고, dfs로 풀어도 되는데 union-find가 더 빠른 것 같다. union-find 알고리즘도 삼성 sw 알고리즘 특강에서 처음 해봤었는데 그때 이후로 까먹었다가 다시 봤다. 이젠 정리해놨으니 잊어먹지 말자. 그리 복잡한 알고리즘도 아니잖아?</p>
