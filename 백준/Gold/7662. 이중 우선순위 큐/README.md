...
### 풀이
 ...
 <p>처음엔 heap을 직접 구현해야 하나 싶었다. 근데 생각해보니 max 값을 구하기 쉬워도 min 값은 왼쪽 자식인지 오른쪽 자식인지 순서가 보장되지 않는다. 그래서 max를 위한 pq, min을 위한 pq을 두어서 각각 연산하고 전체 나온 node 수를 저장하는 multiset이 있어서 top과 비교 후, multiset에 없으면 이미 삭제된 것이므로 pop 하는 방식으로 했는데 5가 2번 나왔고 min에 의해 없어졌는데 multiset에는 5가 하나 남았으니까 max에서 없어지지 않을 수 있다는 문제를 발견했다. </p>

<p>질문 게시판 보니 multiset으로 풀어라는 말이 있어서 multiset의 내부 자료구조를 보니 Red-Black Tree인 것을 확인했다. 그래서 iterator를 돌면 오름차순으로 정렬된다. 그래서 그냥 multiset에서 insert, delete 구현했다. begin(), prev(begin()) 사용했다. </p>
