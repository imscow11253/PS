...
### 풀이 
 ...
 <p>N이 4000이라서 N^2으로 풀 수 있을 것이라고 생각했다. 처음에 간단하게 생각해봤을 때는 그리디로 해도 이중 for문만 나올 것 같아서 그냥 그리디로 했는데 짜다보니 삼중 for문이 나와서 당황했다. 그래도 혹시나 하고 제출해봤는데 역시나 시간초과…ㅋㅋ 

차분하게 로직을 다시 보니 for문 하나를 누적합으로 해결할 수 있을 것 같아서 sum이라는 vector를 하나 더 만들어서 for문 하나 줄이니까 통과했다.</p>
