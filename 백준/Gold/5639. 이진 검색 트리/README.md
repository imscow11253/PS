...
### 출력 
 ...
 <p>풀이 과정을 떠올리기까지 시간이 좀 걸린 문제다. 그래프 문제였으면 바로 떠올렸을텐데 역시 다른 문제는 좀 걸리는 것 같다. 결국 떠올라서 풀긴 했다. 재귀를 사용했다. 김성열 교수님이 재귀의 핵심은 따라 들어가지 않는 것이라고 말씀해주신 것을 기반으로 작성했다. 주어진 vector 기준으로 root, left, right를 나누었고 후위탐색법으로 출력하니 해결했다. 처음에 index 계산을 잘못해서 좀 헤매다가 해결하고 바로 풀었다. </p>
