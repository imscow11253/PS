...
### 출력 
 ...
 <p>킹성열 교수님의 가르침이 떠올라 풀이를 떠올릴 수 있었다. 근데 틀린 풀이였던건 함정. 최단 거리 중에 최소값은 무조건 그 노드간 직선 도로가 될 수 밖에 없다는 것을 알면 된다. 그러면 그 다음 최솟값에도 적용하면 된다. 근데 그렇게 도로를 잇기 전에 존재하는 길들로 최솟값보다 작은 도로가 생기면 -1이 된다. 난 최솟값 도로를 찾는걸 dfs로 돌렸기에 시간초과가 떴다. 이건 그냥 앞선 개념에 입각해서 플로이드-워셜로 풀면된다. GPT가 알려줬다. </p>
