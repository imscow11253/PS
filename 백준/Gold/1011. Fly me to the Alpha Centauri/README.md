...
### 풀이 
 ...
 <p>간단한 문제인데 식을 도출하기 쉽지 않았다. 수열을 직접 써보면 1 2 3 3 4 4 5 5 5 6 6 6 7 7 7 7 8 8 8 8 이런 식으로 길이가 1이 두 번, 길이가 2가 두 번…. 이런 식으로 반복되면서 각 반복마다 value는 1씩 오르는 구조다. 처음엔 반복문으로 구했는데 시간초과가 났다. 아마 T의 값 때문에 각 테스트 케이스마다 O(n)이 아니라 O(1)로 풀어야만 했다. 그래서 수학적으로 한 번에 도출할 수 있는 식을 알아내야 했고 이걸 구하는게 마냥 쉽지는 않았다. 

제곱 수를 기준으로 구했다. 골드인데 골드답지 않게 단순하지만 골드답게 식을 도출하기 어려웠던 문제.
