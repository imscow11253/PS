...
### 풀이 
 ...
 <p>이 문제 풀면서 한 번도 만족했던 적이 없다. 전체적으로 집중이 안되기도 했고, 처음에 BFS로 풀려고 했다. 그래서 자신있게 태그를 봤는데 dp였다... dp로도 푸는데 내가 문제를 완벽하게 이해하지 않아서 대각선 이동을 제대로 구현하지 못했다. 대각선 이동 시 [i-1][j]이랑 [i][j-1] 부분도 벽이면 안된다는 것을 생각해주어야 한다. 그리고 초기화 부분을 좀 잘못했다. 그래서 에라 모르겠다 하고 GPT한테 부탁했다니 해줬다. 더 이상 이 문제를 잡고 있기 싫어서 그냥 GPT 코드 복붙했다. 백준의 몇 번 문제인지도 아는 것을 보니 GPT 성능 미친 것 같다. </p>
