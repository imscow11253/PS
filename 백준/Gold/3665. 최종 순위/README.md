...
### 출력 
 ...
 <p>위상정렬임을 알고 풀었다. 근데 문제를 그냥 봤어도 위상정렬을 떠올렸을 것 같다. 그리고 풀이가 바로 떠올라서 풀었다. 작년에 주어진 대회 순위를 기준으로 초기 그래프를 그릴 수 있다. 각 순위마다 자기보다 작은 순위로의 edge를 가지고 있으면 된다. 순위가 변동되면 그 2명의 edge 방향을 바꾸면 된다. 그리고 위상정렬을 하면 되는데, 차수가 0인 것이 없으면 imposible, 차수가 0인게 여러 개면 ? , 차수가 0인게 계속 1개면 계속 출력하면 된다. </p>
