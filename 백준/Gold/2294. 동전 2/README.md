...
### 풀이 
 ...
 <p>미소가 풀던거 나도 그냥 풀어봤다. 메모이제이션을 사용한 재귀를 사용해서 풀었다. 방문처리를 초기값인지 아닌지로 판단했는데 방문처리가 제대로 명확하게 되지 않아서인지 시간초과가 났다. 그래서 별도의 visit 처리를 해주니 맞았다. </p>
