...
### 풀이 
 ...
 <p>로직을 생각해보면 단순하다. 그냥 먼저 합쳐지는 수는 계속해서 더해져 갈 것이기 때문에 가장 작은 수를 가지는 두개를 먼저 더해야 한다. 그리고 더한 값은 다시 배열에 삽입한다. 

단순 정렬해서 가장 앞의 두 개만 더하는 방법을 생각해보면 n(nlogn) 이라서 시간초과가 날 것이다. 그래서 정렬하지 않고 우선순위 큐를 사용했다. 우선 순위 큐는 업데이트가 logn 이기 때문에 시간 복잡도를 맞출 수 있다.</p>
