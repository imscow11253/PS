...
### 출력 
 ...
 <p>단순 naive하게 2개 선택하는 경우 다 돌면서 더해보고 절댓값이 가장 작은거 선택하면 된다. 하지만 시간초과 난다. 그래서 처음 생각했던 것은 양수, 음수로 나눠서 각각 vector에 두고 50000^2 으로 완전탐색하려고 했다. 시간초과 안날 줄 알았는데 내가 산수계산 실수해서 시간초과 났다. 50000^2은 시간초과 난다. 그래서 새로운 방법을 갈구했고 음수를 모두 양수로 바꿔서 정렬한 다음 바로 옆에 있는 것만 비교하면 될 것 같다는 아이디어가 떠올랐고 그러면 한번의 탐색만으로 해결할 수 있을 것이라 생각해서 풀었더니 됐다. </p>

<p>++++  끝나고 다른 사람 풀이를 살펴봤는데 투포인터로 풀었다. </p>
