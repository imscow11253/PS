...
### 출력 
 ...
 <p>골드 문제이지만 골드문제 같지 않은 문제였다. 생각보다 쉽게 풀렸다. N의 범위가 작아서 시간 복잡도는 고려할 필요 없이 그냥 브루트포스로 풀면 된다. 근데 틀렸다. 왜? 뭔가 논리식에는 문제가 없다고 생각했는데… 하나 걸리는 점은 반복을 얼마나 해야하는지 감이 안잡히는 것 뿐이었다. 

질문 게시판을 보니 최소 10만 이상은 해야 한다고 해서 해봤더니 맞았다. 뭔가 반복문의 범위를 이런식으로 하드코딩 해도 되나 싶었지만 제한 내에서는 어떻게 하든 상관 없을 것 같기도 하다.</p>
