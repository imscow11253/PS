...
### 출력 
 ...
 <p>못 풀었다. dp로 풀어야 된다는 것을 알았으나, 난 2차원 배열로 풀려고 했다. 행을 2개 두고, 0이면 왼발이 움직이는 경우, 1이면 오른발이 움직이는 경우로 생각했다. 그렇게 두고 dp의 저장값을 tuple로 두어서 횟수, 왼발 위치, 오른발 위치로 두었다. 이렇게 하니 생각해야할 경우의 수가 무척 많아서 힘들었다. GPT한테 물어보니 dp문제의 대표격이라고 했다. gpt는 3차원 배열로 풀었다. dp[총 단계 수][왼발 위치][오른발 위치]로 두고 점화식 세워서 풀었다.. standard 문제라고 하니 이런 경우엔 3차원 dp로 풀어야 한다고 외워야겠다. </p>
